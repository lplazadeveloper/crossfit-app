...
export function LoginPage() {
  ...
  // Init Google button once script loads
  useEffect(() => {
    window.handleGoogleCredential = async ({ credential }: { credential: string }) => {
      if (!orgInput.trim()) { setError('Introduce el código de tu organización'); return; }
      setLoading(true);
      setError(null);
      try {
        setOrgSlug(orgInput.trim());
    ...
    const interval = setInterval(() => {
      if (window.google?.accounts) { initBtn(); clearInterval(interval); }
    }, 200);
      ...
      <div className="relative w-full max-w-sm">
        ...
        <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl p-6 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1">
            ...
            <input
              type="text"
              placeholder="ej: lpp-program"
              value={orgInput}
              onChange={e => setOrgInput(e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, ''))}
